...
To run Dinosaur java 1.6 or greater is needed. The only required input data is the MS data in the mzML-format. We recommend using ProteoWizard to produce the mzML.  of Dinosaur 
...
Program execution begin in the Dinosaur class, which handles the outermost program logic. Actual algorithm execution is thought controlled in the FeatureFinder.analyzeMzML method. Dinosaur main parameters are defined in DinosaurParams, and advanced algorithm parameters in DinosaurAdvParams. A DinosaurParams object is instantiated at the start of execution in the Dinosaur class, and passed around where ever parameters are needed.
